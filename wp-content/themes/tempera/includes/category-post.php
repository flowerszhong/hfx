...
class catPostsWidget extends WP_Widget {
	...
	function form($instance) {
		...
		// echo '<label for="' . $this->get_field_name('cat') . '">分类ID:' . get_the_category_list();
		$cat_array = array(
			'selected' => $cat,
			'name' => $this->get_field_name('cat')
		);
		echo '<p><label for="' . $this->get_field_name('cat') . '">请选择分类目录:';
	...
	function widget($args, $instance) {
		...
		$query = new WP_Query("cat=$cat&showposts=$showPosts&orderby=post_date");
		if ($query->have_posts()) {
			echo '<div class="unslider">';
			echo '<ul class="">';
			while ($query->have_posts()) {
				$query->the_post();
				echo '<li><a href="' . get_permalink() . '">' . get_the_title() . '<b>(' . get_the_date('Y-m-d') . ')</b></a></li>';
				// echo '<li><a href="' . get_permalink() . '">';
				// the_post_thumbnail(array(264, 260));
				// echo "</a></li>";
			}
			echo '</ul>';
			echo '</div>';
...
function RandomPostViews() {
	register_widget('catPostsWidget');
}
add_action('widgets_init', 'RandomPostViews');
